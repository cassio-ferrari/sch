...
<nav class="${nav_css_class}" id="navigation" role="navigation">
	...
	<ul aria-label="<@liferay.language key="site-pages" />" class="navbar-nav mr-auto flex-column" role="menubar">
		<#list nav_items as nav_item>
			...
			<li class="${nav_item_css_class} " id="layout_${nav_item.getLayoutId()}" role="presentation">
				<a aria-labelledby="layout_${nav_item.getLayoutId()}" class="nav-link menu-item" ${nav_item_attr_has_popup} href="${nav_item.getURL()}" ${nav_item.getTarget()} role="menuitem"><span><@liferay_theme["layout-icon"] layout=nav_item_layout /> ${nav_item.getName()}</span> ${nav_item_caret}</a>

				<#if nav_item.hasChildren()>
				
					<ul class="child-menu dropdown-menu flex-column" role="menu">
